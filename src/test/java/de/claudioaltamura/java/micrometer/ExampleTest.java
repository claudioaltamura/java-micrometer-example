...
class ExampleTest {
  ...
  void testCounter() {
    ...
    Optional<Counter> counterOptional =
        Optional.ofNullable(meterRegistry.find(XYZProcessor.METRIC_NAME).counter());
  ...
  void testTimer() {
    ...
    Optional<Timer> timerOptional =
        Optional.ofNullable(meterRegistry.find(ZYXProcessor.METRIC_NAME).timer());
